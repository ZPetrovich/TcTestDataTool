...
public class ProjectDataGenerator {
    ...
    public Project generateProject() {
        ...
        if (costLevel != null) {
            ...
            contestTypePrizes.add(createPrize(project.getFirstPlaceCost(), 1, 1, PrizeType.CONTEST_PRIZE, project.getProjectId()));
            contestTypePrizes.add(createPrize(project.getSecondPlaceCost(), 1, 2, PrizeType.CONTEST_PRIZE, project.getProjectId()));
        } else {
            if (isStudio) {
                int numberOfPrizes = getRandomInt(2, 5);
                for (int i = 1; i <= numberOfPrizes; i++) {
                    Prize prize = createPrize(2000 - i * 300, 1, i, PrizeType.CONTEST_PRIZE, project.getProjectId());
        ...
        if (phasesTemplate.contains(PhaseType.MILESTONE_SUBMISSION)) {
            milestoneTypePrize = createPrize(getRandomInt(50, 100), getRandomInt(1, 5), 1, PrizeType.MILESTONE_PRIZE, project.getProjectId());
        ...
        // Observers
        User[] observersUsers = this.config.getObservers();
        if (observersUsers != null) {
            for (User observer : observersUsers) {
                createResource(observer, ResourceRole.OBSERVER, resourceIdGenerator.getNextId(), null, resources);
            }
        }
     ...
     */
    private Prize createPrize(double amount, int numberOfSubmissions, int placement, PrizeType prizeType, long projectId) {
