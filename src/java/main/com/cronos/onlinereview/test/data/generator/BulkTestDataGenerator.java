...
public class BulkTestDataGenerator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (minActiveContestsCount != 0 || maxActiveContestsCount != 0) {
            if (maxProjectFulfillment == 100) {
                throw new IllegalArgumentException("maxProjectFulfillment can not be set to 100 when " +
                                                   "minActiveContestsCount and maxActiveContestsCount are not set to 0");
            }
        }
        ...
        generator.generateBulkTestData(clientsCount, minBillingCount, maxBillingCount, minDirectCount, maxDirectCount, 
                                       ...
                                       averageContestSubmissionsFailureRate);
    ...
    public void generateBulkTestData(int numberOfClients, int minBillingProjectsCountPerClient,
                                     ...
                                     int averageProjectFulfillment, int averageContestSubmissionsFailureRate) 
        throws IOException {
        ...
        for (Client client : this.clients) {
            ...
            for (int i = 1; i <= numberOfBillingProjects; i++) {
                ...
                billingProject.setEndDate(
                    new Date(System.currentTimeMillis() + getRandomInt(100000, 200000) * 24 * 3600 * 1000L));
        ...
        for (BillingProject billingAccount : this.billingProjects) {
            ...
            for (int i = 1; i <= numberOfTcDirectProjects; i++) {
                ...
                if (projectFulfillment != 100) {
                    ...
                    int scale = 1;
                    do {
                        projectTotalContestsCount = 100 * scale;
                        completedContestsCount = projectFulfillment * scale;
                        scale++;
                    } while (activeContestsCount + completedContestsCount > projectTotalContestsCount);
                } else {
                    completedContestsCount = getRandomInt(2, 5);
                    projectTotalContestsCount = completedContestsCount;
                }

                int copilotsCount = getRandomInt(1, 2);
                ...
                totalContestsCount += projectTotalContestsCount;
                totalCompletedContestsCount += completedContestsCount;
            }
        }

        int expectedTotalCompletedContestsCount = averageProjectFulfillment * totalContestsCount / 100;
        int completedContestsCountDiff = expectedTotalCompletedContestsCount - totalCompletedContestsCount;
        if (!this.tcDirectProjects.isEmpty()) {
            while (completedContestsCountDiff != 0) {
                for (int i = 0; completedContestsCountDiff != 0 && i < tcDirectProjects.size(); i++) {
                    TcDirectProject tcDirectProject = tcDirectProjects.get(i);
                    if (completedContestsCountDiff > 0) {
                        tcDirectProject.setCompletedContestsCount(tcDirectProject.getCompletedContestsCount() + 1);
                        completedContestsCountDiff--;
                    } else {
                        tcDirectProject.setCompletedContestsCount(tcDirectProject.getCompletedContestsCount() -1);
                        completedContestsCountDiff++;
                    }
            ...
            while ((generatedContestsCount < numberOfContests)) {
                ...
                if (phasesTemplate.contains(phaseType) || phaseType == null) {
                    ...
                    if (areCompatible(phaseType, projectStatus)) {
                        ...
                        projectGeneratorConfig.setObservers(new User[] {User.YOSHI, User.USER});
